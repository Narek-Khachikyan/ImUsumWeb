...
export default function DashboardHome() {
  ...
  const averageGrade =
    summary.length > 0
      ? summary.reduce((acc, s) => acc + s.average, 0) / summary.length
      : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-main to-blue-dark rounded-2xl p-6 text-white">
        ...
        <p className="mt-2 text-blue-100">
          {user?.role === 'student' && 'Отслеживайте своё расписание, задания и оценки'}
          {user?.role === 'teacher' && 'Управляйте заданиями и оценками учеников'}
          {user?.role === 'director' && 'Управляйте школой, расписанием и персоналом'}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-xl shadow-soft p-6">
          <div className="flex items-center">
            <div className="p-3 bg-blue-100 rounded-lg">
              <svg className="w-6 h-6 text-blue-main" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
            </div>
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-500">Уроков сегодня</p>
        ...
        <div className="bg-white rounded-xl shadow-soft p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-500">Предстоящих заданий</p>
      ...
      {/* Today's schedule */}
      <div className="bg-white rounded-xl shadow-soft p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Расписание на сегодня</h3>
        {todaySchedule.length > 0 ? (
            ...
            {todaySchedule.map((lesson) => (
              ...
              >
                ...
                <div className="flex-1">
                  <p className="font-medium text-gray-900">Урок #{lesson.subject_id}</p>
                  {lesson.room && (
                    <p className="text-sm text-gray-500">Кабинет: {lesson.room}</p>
                  )}
                </div>
              </div>
            ))}
          ...
          <p className="text-gray-500 text-center py-8">На сегодня уроков нет</p>
        )}
      </div>

      {/* Upcoming assignments */}
      <div className="bg-white rounded-xl shadow-soft p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Предстоящие задания</h3>
        {upcomingAssignments.length > 0 ? (
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Сдать до: {new Date(assignment.due_date).toLocaleDateString('ru-RU')}
                  </p>
                </div>
          ...
          <p className="text-gray-500 text-center py-8">Нет предстоящих заданий</p>
        )}
      </div>
    </div>
  );
