  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-gray-900">Мои оценки</h2>
      </div>
      ...
      {summary.length > 0 && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="bg-white rounded-xl shadow-soft p-6">
            <p className="text-sm text-gray-500">Всего оценок</p>
          ...
          <div className="bg-white rounded-xl shadow-soft p-6">
            ...
            <p className="text-3xl font-bold text-green-600 mt-1">
              {summary.length > 0 ? Math.max(...summary.map((s) => s.highest)).toFixed(0) : '-'}
            </p>
          </div>
        </div>
      )}
      ...
      {summary.length > 0 && (
        <div className="bg-white rounded-xl shadow-soft overflow-hidden">
          <div className="px-6 py-4 border-b">
            <h3 className="text-lg font-semibold text-gray-900">По предметам</h3>
          </div>
              ...
              <div key={subject.subject_id} className="px-6 py-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">Оценок: {subject.total_grades}</p>
                  </div>
                  <div className="text-right">
                    ...
                    <p className="text-xs text-gray-500">
                      от {subject.lowest} до {subject.highest}
                    </p>
                  </div>
                </div>
                {/* Progress bar */}
      ...
      <div className="bg-white rounded-xl shadow-soft overflow-hidden">
        <div className="px-6 py-4 border-b">
          <h3 className="text-lg font-semibold text-gray-900">Последние оценки</h3>
        </div>
        {myGrades.length > 0 ? (
          <div className="divide-y">
            {myGrades.slice(0, 10).map((grade) => (
              <div key={grade.id} className="px-6 py-4 flex items-center justify-between">
                <div>
                  <p className="font-medium text-gray-900">Предмет #{grade.subject_id}</p>
                  <p className="text-sm text-gray-500">
                    {new Date(grade.date).toLocaleDateString('ru-RU')} • {grade.grade_type}
                  </p>
                  {grade.comment && (
